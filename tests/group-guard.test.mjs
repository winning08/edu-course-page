...
async function loadGroups() {
  const raw = await readFile(
    new URL("data/activity-groups.json", repoRoot),
    "utf8",
  );
...
const GROUP_PAGES = {
  "machine-learning-algorithms": [
    ...
    "lessons/ai-prediction-lab/index.html",
    "lessons/knn-neighbors/index.html",
    "lessons/decision-tree-score/index.html",
    "lessons/quick-draw/index.html",
  ],
  "ai-evaluation": [
...
test("모든 group은 명시적인 active boolean 필드를 갖는다", async () => {
  ...
  for (const group of data.groups) {
    assert.equal(
      typeof group.active,
      "boolean",
      `${group.id}의 active가 boolean이 아님`,
    );
  }
});

test("모든 활동지를 공개한다", async () => {
  const data = await loadGroups();
  const search = data.groups.find((candidate) => candidate.id === "ai-search");
  const problemCases = data.groups.find(
    (candidate) => candidate.id === "search-problem-cases",
  );
  const algorithms = data.groups.find(
    (candidate) => candidate.id === "machine-learning-algorithms",
  );
  assert.equal(search.active, true);
  assert.equal(problemCases.active, true);
  assert.equal(algorithms.active, true);
  assert.deepEqual(
    algorithms.children.map(({ active }) => active),
    [true, true, true, true],
  );
  assert.deepEqual(
    search.children.map(({ active }) => active),
    [true, true, true, true],
  );
  assert.deepEqual(
    problemCases.children.map(({ active }) => active),
    [true],
  );
  for (const group of data.groups) {
...
test("루트 허브는 hub 스코프 가드를 선언하고, 모든 group에 data-group-card를 붙인다", async () => {
  ...
  for (const group of data.groups) {
    assert.match(
      html,
      new RegExp(`data-group-card="${group.id}"`),
      `허브에 ${group.id}의 data-group-card가 없음`,
    );
  }
});
...
test("group-guard.js의 허브 가드는 fetch 실패 시(catch) 카드를 잠그지 않고 열어 둔다(fail-open)", async () => {
  ...
  const catchBlockMatch = js.match(
    /async function guardHub[\s\S]*?catch \(error\) \{([\s\S]*?)\n\}/,
  );
  assert.ok(catchBlockMatch, "guardHub의 catch 블록을 찾을 수 없음");
  assert.doesNotMatch(
    catchBlockMatch[1],
    /markCardLocked/,
    "허브 fetch 실패 시 카드를 잠그면 안 됨(file:// 등에서 모든 카드가 영구 잠기는 문제)",
  );
  assert.match(
    catchBlockMatch[1],
    /status\.set\(card, "active"\)/,
    "허브 fetch 실패 시 카드를 active로 되돌려(fail-open) 클릭이 통과되어야 함",
  );
});
...
test("모든 unit·lesson 페이지는 공통 가드(pending 초기 상태, 소속 group-id, 준비 중 안내, 홈 복귀 링크)를 갖는다", async () => {
  for (const [groupId, pages] of Object.entries(GROUP_PAGES)) {
    for (const page of pages) {
      ...
      assert.match(
        html,
        /data-guard\("pending"\)|setAttribute\("data-guard",\s*"pending"\)/,
        `${page}에 초기 pending 가드 스크립트가 없음`,
      );
      assert.match(
        html,
        new RegExp(
          `data-guard-scope="page"[^>]*data-guard-group="${groupId}"|data-guard-group="${groupId}"[^>]*data-guard-scope="page"`,
        ),
        `${page}의 data-guard-group이 ${groupId}가 아님`,
      );
      assert.match(
        html,
        /id="guard-blocked"/,
        `${page}에 guard-blocked 안내가 없음`,
      );
      assert.match(
        html,
        /id="guard-content"/,
        `${page}에 guard-content 래퍼가 없음`,
      );
      assert.match(
        html,
        /class="guard-home-link"/,
        `${page}에 홈 복귀 링크가 없음`,
      );
      assert.match(
        html,
        /assets\/group-guard\.js/,
        `${page}에 group-guard.js 로드가 없음`,
      );
      assert.match(
        html,
        /assets\/guard\.css/,
        `${page}에 guard.css 로드가 없음`,
      );
    }
  }
});
  ...
  for (const group of data.groups) {
    for (const child of group.children) {
      assert.equal(
        typeof child.active,
        "boolean",
        `${group.id}의 ${child.id}에 active boolean이 없음`,
      );
    }
  }
});
...
test("ai-evaluation 그룹의 활동 3개는 모두 active=true다(개별 활동 숨김은 현재 쓰지 않음)", async () => {
  ...
  const group = data.groups.find(
    (candidate) => candidate.id === "ai-evaluation",
  );
...
test("모든 활동 페이지(<body>)는 data-guard-lesson으로 자기 자신의 child id를 선언한다", async () => {
  ...
  for (const [page, lessonId] of Object.entries(LESSON_PAGE_TO_ID)) {
    ...
    assert.match(
      html,
      new RegExp(`data-guard-lesson="${lessonId}"`),
      `${page}에 data-guard-lesson="${lessonId}"가 없음`,
    );
  }
});
...
test("모든 활동지 목록 페이지는 각 하위 활동 카드에 data-lesson-card로 child id를 붙인다", async () => {
  for (const [groupId, pages] of Object.entries(GROUP_PAGES)) {
    ...
    for (const child of group.children) {
      assert.match(
        html,
        new RegExp(`data-lesson-card="${child.id}"`),
        `${groupPagePath}에 ${child.id}의 data-lesson-card가 없음`,
      );
    }
  }
});

...
if (el.section) {
  ...
  function showRound(index) {
    ...
    el.costChoiceSummary.innerHTML = `<span>현재 후보 비교</span><strong>${comparison}</strong><p class="cost-choice-hint">f(n)=g(n)+h(n)이 가장 작은 상태를 고릅니다. 같은 값이면 알파벳순으로 고르세요.</p>`;

    el.prompt.textContent = round.pickCandidates.length > 1
      ? "f(n)이 가장 작은 상태로 이어지는 간선을 클릭하세요. f(n)이 같으면 알파벳순으로 고릅니다."
  ...
  function handlePick(clickedId) {
    ...
    if (!outcome.correct) {
      ...
      el.feedback.innerHTML = `<strong>다시 확인해 볼까요.</strong><p>오픈 리스트에서 f(n)이 가장 작은 상태를 찾으세요. 같은 값이 여러 개면 알파벳순으로 먼저인 상태를 고릅니다.</p>`;
